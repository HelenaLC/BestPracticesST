...
For large-scale datasets (say, 100,000s of cells), *.h5* files allow for out-of-memory representation of count matrices, represented as `r BiocStyle::Biocpkg("DelayedArray")`s in R.
...
R-interfaces to *.zarr* are provided through `r BiocStyle::Biocpkg("Rarr")` (Bioc) and `r BiocStyle::CRANpkg("pizarr")` (CRAN).
...
A summary of the `SpatialExperiment` object structure is shown in the Figure \@ref(fig:spe). 
...
```{r spe, echo = FALSE, out.width = "100%", fig.cap = "Overview of the `SpatialExperiment` data class for storing and manipulating spatial transcriptomics datasets within the Bioconductor framework."}
...
`r BiocStyle::Githubpkg("satijalab/seurat")` and `scanpy` [@Wolf2018-scanpy] provide purportedly comprehensive single-cell analysis pipelines in R and Python, respectively.
Similarly, `r BiocStyle::Githubpkg("drieslab/Giotto")` [@Chen2023-Giotto] and `squidpy` [@Palla2022-squidpy] provide all-in-one frameworks for analyzing spatial omics data.

The [`SpatialData`](https://spatialdata.scverse.org) [@Marconato2025-SpatialData] framework offers a suit of modules for loading, handling, as well as (static and interactive) visualization of divers spatial omics data (spanning imaging- and sequencing based data and different modalities).
...
```{r spd, echo = FALSE, out.width = "100%", fig.cap = "Overview of the scverse/Python `SpatialData` framework."}
